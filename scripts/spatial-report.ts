...
writeFileSync(
  `${dest}/index.html`,
  `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Ruinweavers · Spatial Design 0.3</title><style>body{font:16px/1.5 system-ui;background:#e8e6db;color:#233b42;max-width:1300px;margin:32px auto;padding:0 22px}a{color:#195c7a}img{max-width:100%;height:auto}.plots{display:flex;overflow:auto;gap:12px}figure{margin:0;min-width:240px;flex:1}figcaption{font-size:13px}section{margin:36px 0}video{max-width:100%}</style><h1>The Broken Court — historical spatial study</h1><p><a href="/?quality=lightweight">Play the five-court run</a> · <a href="/?scene=run-classic">Original Guardian run</a></p><p>Authored plans and synthetic trajectories are diagnostics, not gameplay captures or a layout score. Same mixed enemy composition for ordinary probes; identical Guardian rules for the final room. 200ms delayed observations, imperfect aim and the existing keyboard motor. Empty regions can reflect policy bias. Purple dots show field placement.</p><h2>Eight candidate plans</h2><img src="candidate-plans.svg" alt="Eight room plans">${sections.join("")}<section><h2>Current gameplay</h2><p>This historical study retains its plans and diagnostic traces. Older footage predates the wall-flicker fix and has been retired. <a href="/topology/index.html">Watch current gameplay and video</a> or <a href="/history/index.html">inspect selected visual history</a>.</p></section><h2>Greybox comparisons</h2><p>${["terrace", "broken", "archive"].map((id) => `<a href="/?scene=trial/mixed&room=${id}&art=off">${ROOMS[id].name}</a>`).join(" · ")}</p></html>`,
);
