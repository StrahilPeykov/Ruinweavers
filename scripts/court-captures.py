...
Usage: python scripts/court-captures.py CAPTURE_DIR RECORDED_WEBM [START_SECONDS]
...
subprocess.run(['ffmpeg','-y','-ss',start,'-i',str(video),'-t','15',
...
<small>Build BUILD_LABEL. Actual two-client local WebRTC gameplay. Normal health and active enemies; scripted choices executed through real keyboard/mouse inputs. Lightweight, native Chrome / Intel UHD on one Windows computer. Video recording affects frame timings; performance was measured separately. No concept images.</small>
...
<p>Create co-op, copy the room code, partner joins, both Ready. Controls and settings include Standard/Lightweight and the preserved development comparisons.</p></main></html>'''.replace('BUILD_LABEL',build),encoding='utf-8')
