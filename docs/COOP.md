...
For two people on separate computers, each needs this same checkout and runs the same command locally. Leave **Connection options → Public Nostr · internet** selected on both. One clicks **Create co-op**, shares the complete RW code privately, and the other enters it and clicks **Join co-op**. Both press **Ready**. Both must be ready again between encounters and to restart after victory/defeat. The top Restart button during combat abandons the attempt and waits for the partner's readiness. Joining an active attempt is not supported.

For two windows on this computer, the same public path works, or choose **Local relay · same machine test** on both. `npm run dev` starts the official Trystero WebSocket signaling relay on loopback port 4174 alongside Vite on 5173. Local relay is not an internet/LAN hosting service. The default public path uses free external signaling/STUN; no account, TURN subscription, deployment or public repository is involved.
...
Inputs older than 250 ms stop movement/held casting and clear pending intent. Sequence numbers and encounter epochs reject replayed/obsolete input. Focus loss releases controls. Six seconds without an accepted input/snapshot heartbeat stops the attempt; leaving/disconnecting also freezes it with an explanation. Missing rooms time out after 25 seconds. Return to solo or create/join a fresh room; no reconnect recovery or host migration. Keep the host browser running in the foreground: OS/browser background throttling can stall it. No TURN fallback is configured, so restrictive NAT/firewalls can prevent joining. Public relay availability is outside this local project.

**Verified:** actual WebRTC with two independent Chromium clients on one Windows machine, using both loopback signaling and public Nostr discovery. **Not verified:** two physical computers on different networks, physical trackpad/keyboard comfort, subjective guest latency, or remote GPU performance. The public signaling test is still a same-machine transport test.
