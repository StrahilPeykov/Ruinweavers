...
Spatial Design 0.3 keeps this direction and the existing GLBs. The five selected RoomSpecs determine actual dimensions, fitted cover bounds, materials and landmarks. Instanced floor inlays follow authored support; no flat decorative floor crosses diagnostic gaps. Stone caps/pigment bands dress solid boundaries, the rotunda lens sits on central masonry, and the repair yard's vessels sit on its non-playable block. The final crest/bindings still resolve only on authoritative victory. Existing art recipes/proofs remain available through explicit links; no new assets or pipeline dependency. Source bounds scale cover art to each collider. This replaces the old identical 24×22 dressing described below for the normal run.

Guardian 0.1 adds one original painted-stone/bronze articulated core and a shared physical plate GLB to the run's existing illustrated library (12 runtime assets in run/Guardian scenes; original study scenes retain ten). `node scripts/art/build.mjs --guardian` rebuilds only these assets and its separate validation report. Authority drives three preparations, impacts, a short binding release and collapse; seven reusable danger meshes remain above spell effects. No new palette, biome or pipeline. Asset dimensions/group counts and current performance: [GUARDIAN](GUARDIAN.md).
...
Historical pre-spatial rollout: `render/rooms.ts` mapped encounter names to five composition roles on repeated terrain. `run-classic` preserves that arrangement. The current run uses explicit RoomSpec presentation instead. One shared GLB library is loaded per client; room instances are disposed/rebuilt at transitions, never five live scenes. Final bindings fold down and the crest settles only on authoritative victory; terminal framing returns toward court center. A quiet local synthesized completion chord supports the resolution.
...
Current handoff: `/topology/index.html` contains post-wall-fix gameplay. `/history/index.html` keeps two labelled art-comparison sheets; `/spatial/index.html` preserves plans/traces. Obsolete public clips and repeated art captures are retired, recoverable at commit `dc4bf50`. Historical descriptions below document what was captured at that time, not what is currently shipped. The GLB/source pipeline and playable proof scenes remain intact. Historical media packagers now write to ignored `artifacts/media-archive/raw/`; current media uses `scripts/topology-captures.py`. See [MEDIA](MEDIA.md).
