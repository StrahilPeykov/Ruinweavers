...
Current handoff: `/broken-court/index.html` has the five-room contact sheet, normal gameplay frame and 15-second co-op excerpt. The separate complete recorded run used normal HP/active enemies and actual controls/choices; its capture overhead is excluded from performance evidence. `scripts/court-captures.py CAPTURE_DIR WEBM START_SECONDS` packages selected screenshots and a real excerpt with Pillow/ffmpeg after tests finish. Old study pages are explicitly marked historical; their media and source assets remain intact.
