...
Current handoff: `/spatial/index.html` contains selected-room gameplay, candidate plans and diagnostic trajectories. `scripts/package-spatial.py RUN_DIR WEBM` packages normal viewports and a short real-input excerpt after performance work; `scripts/spatial-report.ts` generates the plans/page from RoomSpecs and local traces. `/broken-court/index.html` retains the historical pre-spatial run captures, made through `scripts/court-captures.py`. Recording overhead is labelled in current measurements; old media/source assets remain intact.
